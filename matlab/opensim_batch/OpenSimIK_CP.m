function OpenSimIK_CP()
    tic
    baseDir = 'F:\AlterG\CP\Data';
    contents = dir(baseDir);
    dirFlags = [contents.isdir];
    subfolders = contents(dirFlags);
    suffixes = {'PreHigh', 'PostHigh', 'PreLow', 'PostLow'};
    ...
    % Skip the first 6 subfolders
    startIndex = 9; % Start from the 7th folder (skip first 6)
    if length(subfolders) < startIndex
        error('There are fewer than 6 subfolders in the directory.');
    end

    % Generate Scaled things 
    for k = startIndex : length(subfolders)
        subfolderName = subfolders(k).name;
        ...
        % Display the name of the 7th folder
        disp(['7th subfolder is: ', subfolderName]);
       ...
       % Iterate over each suffix 
        for j = 1 : length(suffixes)
            ...
            SetupPath = fullfile(baseDir, subfolderName, suffix, 'Setup');
            IKResultsPath = fullfile(baseDir, subfolderName, suffix, 'IKResults');
            GaitPath = fullfile(baseDir, subfolderName, suffix, 'Gait');
            ScaledModelPath = fullfile(baseDir, subfolderName, suffix, 'ScaledModel');
                  
            % Check for the existence of the scaled model; if not, scale it
            scaledModelFiles = dir(fullfile(ScaledModelPath, '*.osim'));
            
            
            % Skip OpenScale function if k > 12
            if k > 12
                disp(['Skipping OpenScale for participant ', subfolderName, suffix]);
            elseif isempty(scaledModelFiles)
                OpenScale(baseDir, subfolderName, suffix)
            else
                disp(['Participant ', subfolderName, suffix, ' Scaled'])
            end
    ...
    for k = 1 : length(subfolders)
        ...
        for j = 1 : length(suffixes)
            ...
            if exist(GaitPath, 'dir') == 7
                ...
                participantID = [subfolderName, '_', suffix];
                
                % Gait
                saveDir = 'F:\AlterG\CP\InverseKinematics\Gait';
...
function OpenScale(baseDir, subfolderName, suffix)
    % Initiate libraries 
    ...
    participantDir = fullfile(baseDir, subfolderName, suffix);
    calibrationFilePath = '';
    calibrationFolder = '';
    ...
    opts = detectImportOptions(demographicDataFile, 'Sheet', 'CerebralPalsy');
    demographicData = readtable(demographicDataFile, opts);

    % Extract the subject ID from the subfolder name
    parts = strsplit(subfolderName, filesep);
    subjectID = str2double(parts{1});

    % Find the row corresponding to the subject ID and suffix
    subjectRow = demographicData(demographicData.ID == subjectID & strcmp(demographicData.Suffix, suffix), :);
    ...
    outputModelFileName = fullfile(resultsDir, ['ScaledModel_' subfolderName '_' suffix '.osim']);
    ...
    xmlFileName = fullfile(resultsDir, ['ScaleSetup_' subfolderName '_' suffix '.xml']);
